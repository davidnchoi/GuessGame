...
class GuessingGameGUI:
    def __init__(self, master):
        ...
        # UI Elements
        self.title_label = tk.Label(master, text="Multiplayer Number Guessing Game!", font=("Arial", 30, "bold"))
        self.title_label.grid(row=0, column=0, pady=10)

        self.ip_label = tk.Label(master, text="Server IP:", font=("Arial", 16))
        self.ip_label.grid(row=1, column=0, pady=5, sticky="e")
        self.ip_entry = tk.Entry(master, width=15, font=("Arial", 16), justify='center')
        self.ip_entry.grid(row=1, column=0, pady=5, sticky="w", padx=(200,0))
        self.ip_entry.insert(0, "127.0.0.1") # Default to localhost

        self.port_label = tk.Label(master, text="Port:", font=("Arial", 16))
        self.port_label.grid(row=2, column=0, pady=5, sticky="e")
        self.port_entry = tk.Entry(master, width=10, font=("Arial", 16), justify='center')
        self.port_entry.grid(row=2, column=0, pady=5, sticky="w", padx=(200,0))
        self.port_entry.insert(0, "65432") # Default port

        self.connect_button = tk.Button(master, text="Connect to Server", command=self.connect_to_server, font=("Arial", 14))
        self.connect_button.grid(row=3, column=0, pady=10)

        self.status_label = tk.Label(master, text="Not Connected", font=("Arial", 14), fg="gray")
        self.status_label.grid(row=4, column=0, pady=5)

        self.instruction_label = tk.Label(master, text="", font=("Arial", 18))
        self.instruction_label.grid(row=5, column=0, pady=5)
        
        self.feedback_label = tk.Label(master, text="", font=("Arial", 16))
        self.feedback_label.grid(row=6, column=0, pady=10)

        self.guess_entry = tk.Entry(master, width=10, font=("Arial", 16), justify='center')
        ...
        self.guess_button = tk.Button(master, text="Guess", command=self.send_guess, font=("Arial", 14))
        self.guess_button.grid(row=8, column=0, pady=10)
        self.guess_button.config(state='disabled') # Disable until connected
        
        self.other_players_feedback = tk.Text(master, height=5, width=50, font=("Arial", 12), state='disabled')
        self.other_players_feedback.grid(row=9, column=0, pady=10)
        
        # Center the widgets using grid_rowconfigure and grid_columnconfigure
        # We already did this, now ensure elements are not sticky to edges if not needed
    ...
    def _update_feedback_label(self, message):
        if "Welcome to the multiplayer guessing game!" in message:
            self.instruction_label.config(text="I'm thinking of a number between 1 and 100.")
            ...
            if "Client" in message: # This is feedback for another player
                ...
                self.feedback_label.config(text=message, fg="black")
            ...
            self.instruction_label.config(text="A new game has started! I'm thinking of a number between 1 and 100.")
            self.feedback_label.config(text="")
            self.guess_entry.config(state='normal')
            self.guess_button.config(state='normal')
            self.guess_entry.focus_set()
